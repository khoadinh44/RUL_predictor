...
def main(opt, train_data, train_label, test_data, test_label):
  ...
  if opt.rul_train:
    network.compile(optimizer=AngularGrad(1e-4), loss=tf.keras.losses.MeanAbsolutePercentageError(), metrics=['mae', r2_keras, tf.keras.metrics.mean_squared_error], run_eagerly=True) # https://keras.io/api/losses/
  network.summary()
